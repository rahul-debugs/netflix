    ...
    <nav className="bg-gray-900 text-white shadow-lg fixed top-0 w-full z-10 border-b border-gray-700">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <a href="#" className="text-2xl font-extrabold text-white no-underline">
            Netstram
          </a>
          ...
          <div className="hidden lg:flex lg:space-x-8">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="relative text-white text-lg font-semibold no-underline py-2
        ...
        <div className="px-2 pt-2 pb-3 space-y-1 bg-gray-900 border-t border-gray-700">
          ...
          {navItems.map((item) => (
            <a
              ...
              href={item.href}
              onClick={toggleMenu}
              className="block px-3 py-2 rounded-md text-base font-medium text-white hover:bg-gray-800 no-underline"
            >
              {item.name}
            </a>
          ))}
